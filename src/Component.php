...
class Component extends BaseObject
{
    ...
    public function __set($name, $value)
    {
        ...
        if (method_exists($this, $setter)) {
            ...
            $this->attachBehavior($name, $value instanceof Behavior ? $value : Helper::createObject($value));
    ...
    public function hasEventHandlers($name)
    {
        ...
        foreach ($this->_eventWildcards as $wildcard => $handlers) {
            if (!empty($handlers) && Helper::matchWildcard($wildcard, $name)) {
                return true;
            }
        }
    ...
    public function trigger($name, Event $event = null)
    {
        ...
        $eventHandlers = [];
        foreach ($this->_eventWildcards as $wildcard => $handlers) {
            if (Helper::matchWildcard($wildcard, $name)) {
    ...
    private function attachBehaviorInternal($name, $behavior)
    {
        if (!($behavior instanceof Behavior)) {
            $behavior = Helper::createObject($behavior);
        }
        if (is_int($name)) {
